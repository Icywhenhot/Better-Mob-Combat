    ...
    public static void playSound(ServerLevel world, Entity entity, WeaponAttributes.Sound sound) {
        if (sound != null) {
            try {
                ...
                SoundEvent soundEvent = BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse(sound.id()));
                float distance = soundEvent.getRange(sound.volume());
                BMCNetwork.playMobAttackSound(world, entity.getId(), entity.getX(), entity.getY(), entity.getZ(), sound.id(), sound.volume(), pitch, RNG.nextLong(), distance, entity.level().dimension());
